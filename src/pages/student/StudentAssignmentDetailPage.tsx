...
export function StudentAssignmentDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    {assignment.learningContent ? <Card><CardContent className="space-y-3 p-5"><div className="flex items-center gap-2"><BookOpen className="size-5 text-primary" /><h2 className="font-semibold text-text">{assignment.learningContent.title}</h2></div><p className="text-sm text-text-muted">{assignment.learningContent.description ?? 'Learning content for this assignment.'}</p>{assignment.learningContent.content && <p className="whitespace-pre-wrap text-sm leading-6 text-text">{assignment.learningContent.content}</p>}</CardContent></Card> : <Card><CardContent className="p-5"><p className="text-sm text-text-muted">Learning content has not been added for this assignment yet.</p></CardContent></Card>}
